...
# from dct_idct import dct_idct_stuff
...
class YangAttack:
  ...
  def perturb(self, x_nat, y, large_num_of_attacks):
    ...
    for rep in range(large_num_of_attacks):
      # Sagar part
      # x_dct = dct_idct_stuff(x_nat)
      # x = x_nat + np.random.uniform(-self.epsilon, self.epsilon, x_nat.shape)
      shape = x_nat.shape
      a=list(np.random.uniform(-self.epsilon,0,1000))
      b= [0] * 1000
      c = [-self.epsilon] * 700
      d = [self.epsilon] * 100
      a.extend(b)
      ...
      x = x_nat+shaped_array
      x = np.clip(x, 0, 1) # ensure valid pixel range
      # x = np.clip(x_dct, 0, 1) # ensure valid pixel range
